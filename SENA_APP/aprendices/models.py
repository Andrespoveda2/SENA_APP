...
class Aprendiz(models.Model):
    ...
    telefono = models.IntegerField(null=True, blank=True) # Añadido blank=True
    correo = models.EmailField(null=True, blank=True) # Añadido blank=True
    # CAMBIO CRÍTICO: Renombrado a 'fecha_nacimiento' (minúscula) y añadido blank=True
    fecha_nacimiento = models.DateField(null=True, blank=True)
    ciudad = models.CharField(max_length=100, null=True, blank=True) # Añadido blank=True
    # CAMBIO CRÍTICO: Añadido null=True y blank=True para que sea opcional en el form
...
# Los modelos Curso, InstructorCurso, AprendizCurso quedan como estaban previamente
# Asegúrate de que los ForeignKeys a 'instructores.Instructor' y 'programas.Programa'
# estén definidos como cadenas si esas apps no están arriba de 'aprendices' en INSTALLED_APPS.
...
class Curso(models.Model):
    ...
    programa = models.ForeignKey(
        Programa, # Usamos el modelo directamente porque ya está importado
        on_delete=models.CASCADE,
        verbose_name="Programa de Formación",
    )
    instructor_coordinador = models.ForeignKey(
        Instructor, # Usamos el modelo directamente porque ya está importado
        on_delete=models.CASCADE,
...
class InstructorCurso(models.Model):
    instructor = models.ForeignKey(Instructor, on_delete=models.CASCADE) # Usamos el modelo directamente
